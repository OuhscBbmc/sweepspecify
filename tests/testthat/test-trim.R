...
test_that("trim_numeric -good", {
  ...
  observed_1  <- trim_numeric(x, bounds)
  observed_2  <- trim_numeric(x, bounds, replacement)
...
test_that("trim_numeric -degenerate", {
  ...
  expect_equal(trim_numeric(NA_real_, bounds), NA_real_)
  expect_equal(trim_numeric(numeric(0), bounds), numeric(0))
})
